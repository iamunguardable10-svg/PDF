-- ============================================================
-- CLUB OS — SAFE MIGRATION (additive only, no data loss)
-- Creates missing tables, adds missing columns, fixes RLS.
...
drop policy if exists "Org members read all"          on organization_memberships;
drop policy if exists "Users can add themselves"      on organization_memberships;
drop policy if exists "Owners can add others"         on organization_memberships;

create policy "Users read own memberships"
  on organization_memberships for select
  using (user_id = auth.uid()::text);

create policy "Org members read all"
  on organization_memberships for select
  using (
    exists (
...
drop policy if exists "Org members can create departments"  on departments;
...
-- ── 5. ATT_TEAMS — add missing columns ───────────────────────────────────────

create table if not exists att_teams (
  id              text primary key,
...
drop policy if exists "Trainers manage their own teams"        on att_teams;
drop policy if exists "Athletes can read teams they belong to" on att_teams;
drop policy if exists "Anyone can read teams for invite flow"  on att_teams;
...
create table if not exists att_team_members (
  id                text primary key,
...
drop policy if exists "Trainers manage members of their teams" on att_team_members;
drop policy if exists "Athletes read/write their own membership" on att_team_members;

create policy "Trainers manage members of their teams"
  on att_team_members for all
  using (
    exists (select 1 from att_teams where id = att_team_members.team_id and trainer_id = auth.uid()::text)
  )
  with check (
    exists (select 1 from att_teams where id = att_team_members.team_id and trainer_id = auth.uid()::text)
  );

create policy "Athletes manage their own membership"
  on att_team_members for all
  using  (athlete_user_id = auth.uid()::text)
  with check (athlete_user_id = auth.uid()::text);


-- ── 7. ATT_SESSIONS — add missing columns ────────────────────────────────────
...
drop policy if exists "Trainers manage their sessions"         on att_sessions;
drop policy if exists "Athletes read sessions they are in"     on att_sessions;
...
-- ── 8. ATT_SESSION_ATHLETES ───────────────────────────────────────────────────

create table if not exists att_session_athletes (
  id                text primary key,
...
drop policy if exists "Trainers manage session athletes"         on att_session_athletes;
drop policy if exists "Athletes read their own session entries"  on att_session_athletes;

create policy "Trainers manage session athletes"
  on att_session_athletes for all
  using (
    exists (select 1 from att_sessions where id = att_session_athletes.session_id and trainer_id = auth.uid()::text)
  )
  with check (
    exists (select 1 from att_sessions where id = att_session_athletes.session_id and trainer_id = auth.uid()::text)
  );
...
-- Allow 'late' in override_status (drop old constraint, add new)
alter table att_records drop constraint if exists att_records_override_status_check;
alter table att_records add constraint att_records_override_status_check
  check (override_status in ('maybe','no','late'));

alter table att_records enable row level security;

drop policy if exists "Trainers manage records for their sessions" on att_records;
drop policy if exists "Athletes manage their own records"          on att_records;

create policy "Trainers manage records for their sessions"
  on att_records for all
  using (
    exists (select 1 from att_sessions where id = att_records.session_id and trainer_id = auth.uid()::text)
  )
  with check (
    exists (select 1 from att_sessions where id = att_records.session_id and trainer_id = auth.uid()::text)
  );
...
drop policy if exists "Anyone can read facility units"               on facility_units;
drop policy if exists "Trainers can manage units of their facilities" on facility_units;

create policy "Anyone can read facility units"
  on facility_units for select using (true);

create policy "Trainers can manage units of their facilities"
  on facility_units for all to authenticated
  using (
    exists (
      select 1 from facilities f
      where f.id = facility_units.facility_id
        and (
          exists (select 1 from att_teams where organization_id = f.organization_id and trainer_id = auth.uid()::text)
          or exists (select 1 from organization_memberships where organization_id = f.organization_id and user_id = auth.uid()::text)
        )
    )
  )
  with check (
...
-- ── 12. FACILITY BLACKOUTS ────────────────────────────────────────────────────

create table if not exists facility_blackouts (
  id               text primary key,
  facility_id      text not null references facilities(id) on delete cascade,
...
drop policy if exists "Anyone can read blackouts"    on facility_blackouts;
drop policy if exists "Trainers can manage blackouts" on facility_blackouts;
...
create policy "Trainers can manage blackouts"
  on facility_blackouts for all to authenticated
  using (
    exists (
      select 1 from facilities f
...
-- ── 13. EVENT FACILITY BOOKINGS ───────────────────────────────────────────────

create table if not exists event_facility_bookings (
...
drop policy if exists "Anyone can read bookings"                    on event_facility_bookings;
drop policy if exists "Trainers manage bookings for their sessions" on event_facility_bookings;

create policy "Anyone can read bookings"
  on event_facility_bookings for select using (true);

create policy "Trainers manage bookings for their sessions"
  on event_facility_bookings for all to authenticated
  using (
    exists (select 1 from att_sessions where id = event_facility_bookings.session_id and trainer_id = auth.uid()::text)
  )
  with check (
    exists (select 1 from att_sessions where id = event_facility_bookings.session_id and trainer_id = auth.uid()::text)
  );
...
drop policy if exists "Athletes can send join requests"      on att_join_requests;
drop policy if exists "Athletes read own requests"           on att_join_requests;
drop policy if exists "Trainers read requests for their teams" on att_join_requests;
drop policy if exists "Trainers update requests for their teams" on att_join_requests;

create policy "Athletes can send join requests"
  on att_join_requests for insert to authenticated
  with check (user_id = auth.uid()::text);
...
create policy "Trainers read requests for their teams"
  on att_join_requests for select
  using (
    exists (select 1 from att_teams where id = att_join_requests.team_id and trainer_id = auth.uid()::text)
  );
...
-- ── 15. INDEXES (only if not exist) ──────────────────────────────────────────

create index if not exists att_sessions_trainer_id_idx      on att_sessions(trainer_id);
create index if not exists att_sessions_team_id_idx         on att_sessions(team_id);
create index if not exists att_sessions_datum_idx           on att_sessions(datum);
create index if not exists att_team_members_team_id_idx     on att_team_members(team_id);
create index if not exists att_team_members_user_id_idx     on att_team_members(athlete_user_id);
create index if not exists att_records_session_id_idx       on att_records(session_id);
create index if not exists att_records_user_id_idx          on att_records(athlete_user_id);
create index if not exists att_session_athletes_session_idx on att_session_athletes(session_id);
create index if not exists att_session_athletes_user_idx    on att_session_athletes(athlete_user_id);
...
create index if not exists facility_bookings_unit_idx       on event_facility_bookings(facility_unit_id);
create index if not exists facility_units_facility_idx      on facility_units(facility_id);
...
language sql security definer as $$
  select m.* from att_team_members m
...
-- ── FERTIG ────────────────────────────────────────────────────────────────────
-- Bestehende Daten wurden nicht gelöscht.
-- Neue Tabellen und fehlende Policies wurden hinzugefügt.
