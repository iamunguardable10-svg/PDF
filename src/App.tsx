...
function App() {
  ...
  const goBackToLanding = () => {
    localStorage.removeItem('fitfuel_guest');
    localStorage.removeItem('fitfuel_seen_landing');
    localStorage.removeItem('fitfuel_tour_done');
    localStorage.removeItem('teamload_tour_done');
    clearAppMode();
    setIsGuest(false);
    setAppMode(null);
    setShowLanding(true);
  ...
  if (showLanding && !loggedInUser) return <><LandingPage onStart={dismissLanding} onGuest={handleGuestMode} />{showAuthModal && CLOUD_ENABLED && <AuthScreen onClose={() => setShowAuthModal(false)} onGuest={() => { handleGuestMode(); setShowAuthModal(false); }} onLoggedIn={() => { handleLoggedIn(); dismissLanding(); setShowAuthModal(false); }} />}</>;
  if (!appMode) return <RoleSelectScreen userId={loggedInUser?.id} userName={profile.name || loggedInUser?.email || ''} userSport={profile.sport || ''} onSelect={handleSelectMode} onJoined={() => navigate('/athlete', { replace: true })} onBackToLanding={goBackToLanding} />;
