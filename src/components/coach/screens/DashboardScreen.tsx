  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-gray-700 bg-gray-900/80 p-5">
        <p className="text-xs font-semibold uppercase tracking-wider text-violet-300">TeamLoad Preview</p>
        <h2 className="mt-2 text-2xl font-black text-white">Coach Operations Dashboard</h2>
        <p className="mt-2 text-sm text-gray-400">
          Daily overview for sessions, availability, team structure and load risk. This is a safe first preview that only changes the dashboard screen.
        </p>
      </section>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Stat label="Departments" value={loading ? '...' : String(departments.length)} sub="total" color="amber" />
      </div>
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        ...
        <MiniCard label="Risk" value="ACWR" text="Risk summary will come from the existing load monitor data." />
        <MiniCard label="Actions" value="Alerts" text="No-show, low availability and repeated absence alerts come next." />
      </section>
