    ...
    <div className="bg-gray-900/50 rounded-3xl border border-gray-800 overflow-hidden">
      ...
      {!open && (
        <div className="px-4 sm:px-5 pb-3 text-xs text-gray-600">
          14-Tage-Prognose — Plan + Wochentagmuster — aufklappen für Details
        </div>
      )}
      ...
      <p className="text-xs text-gray-500 leading-relaxed">
        14-Tage-Prognose: geplante Einheiten ({plannedCount}) werden mit historischem Wochentagmuster kombiniert.
        Tage ohne Plan werden aus dem Median desselben Wochentags (letzte 12 Wochen) geschätzt.
        ACWR = Acute (7d-Ø) ÷ Chronic (28d-Ø) — optimale Zone 0.8–1.3.
      </p>
        ...
        <div>
          ...
          <div className="space-y-1">
            {projected.map(p => {
              ...
              const basisColor =
                p.forecastBasis?.startsWith('Plan') ? 'text-violet-400' :
                p.forecastBasis === 'Ruhetag (historisch)' ? 'text-gray-600' :
                'text-gray-500';
              return (
                <div key={p.datum} className="py-1.5 border-b border-gray-800/50 last:border-0 space-y-0.5">
                  ...
                  {p.forecastBasis && (
                    <div className={`text-xs pl-28 ${basisColor}`}>{p.forecastBasis}</div>
                  )}
                </div>
              );
            })}
